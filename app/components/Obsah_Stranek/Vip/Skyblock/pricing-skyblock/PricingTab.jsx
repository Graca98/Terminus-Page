  ...
  const vipContent = (
    <div id="vip">
      ...
      <ul className="list-inside">
        ...
        <li>
          /is biome - změna biomi ostrova (Desert, Nether, Jungle, Swamp, Snowy
          Plains)
        </li>
        <li>/is settings - Ovládání počasí a růstu rostlin/stromů</li>
        <li>
          /repair (&lt;$1000, pouze předměty bez enchantů, co hodinu, funguje
          pouze na VIP)
        </li>
        <li>
  ...
  const evipContent = (
    <div id="evip">
      ...
      <ul className="list-inside">
        ...
        <li>
          /repair all (zdarma, každý předmět, co hodinu, funguje pouze na EVIP)
        </li>
        <li>
          /ah sellinventory - Možnost tvorby balíčku k prodeji (takový shulker
          bez shulkeru)
        </li>
        ...
        <li>
          /day rewol - Jednou za 20 hodin umožní vyměnit libovolný děník za jiný
        </li>
        <li>
          /kit evip - Jednorázový EVIP balíček (více informací na cedulkách)
        </li>
      </ul>
    </div>
  );
      ...
      <div className="flex justify-center">
        <div className="tabs">
          <div
            ...
            className={`tab tab-bordered px-6 text-gray-900 ${activeTab === "vip" ? "tab-active" : ""}`}
          >
            <Link href="/vip/skyblock#vip">VIP</Link>{" "}
            {/* Slouží ke scrollu na div */}
          </div>
          <div
            onClick={() => setActiveTab("evip")}
            className={`tab tab-bordered px-6 text-gray-900 ${activeTab === "evip" ? "tab-active" : ""}`}
          >
            <Link href="/vip/skyblock#evip">EVIP</Link>
          </div>
          <div
            onClick={() => setActiveTab("donator")}
            className={`tab tab-bordered px-6 text-gray-900 ${activeTab === "donator" ? "tab-active" : ""}`}
          >
            <Link href="/vip/skyblock#donator">DONATOR</Link>
          </div>
        </div>
      </div>
